...
@section('content')
    <div class="row">
        <div class="col-md-8">
            <div class="card grade-card">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-edit"></i> Class Information</h3>
                ...
                <form action="{{ route('grades.update', $grade->id) }}" method="POST" id="gradeForm">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                                           id="name" placeholder="e.g., Class 1, Class 10, Nursery" value="{{ old('name', $grade->name) }}" required>
                                    @error('name')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                    <small class="form-text text-muted">Enter the class name as it will appear in reports</small>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="section" class="form-control @error('section') is-invalid @enderror" 
                                           id="section" placeholder="e.g., A, B, C" value="{{ old('section', $grade->section) }}" maxlength="1">
                                    @error('section')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="capacity" class="form-control @error('capacity') is-invalid @enderror" 
                                           id="capacity" placeholder="e.g., 40" value="{{ old('capacity', $grade->capacity ?? 40) }}" min="1" max="100">
                                    @error('capacity')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description" rows="3" 
                                              ...
                                              placeholder="Enter class description, curriculum details, or special notes...">{{ old('description', $grade->description) }}</textarea>
                                    @error('description')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="grade-color-selector">
                                        @for($i = 1; $i <= 12; $i++)
                                            <div class="color-option grade-{{ $i }}" data-grade="{{ $i }}" title="Class {{ $i }}">
                                                {{ $i }}
                                            </div>
                                        @endfor
                                    </div>
                                    <input type="hidden" name="grade_theme" id="gradeTheme" value="{{ old('grade_theme', $grade->grade_theme ?? 1) }}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                                        <option value="active" {{ old('status', $grade->status ?? 'active') == 'active' ? 'selected' : '' }}>Active</option>
                                    ...
                                    @error('status')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                    <small class="form-text text-muted">Set the current status of this class</small>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-md-4">
                ...
                <div class="card-body">
                    <ul class="list-unstyled mb-0">
                        <li class="mb-2">
                            ...
                            Use clear, consistent naming (e.g., "Class 1", "Class 10A")
                        </li>
                        <li class="mb-2">
                            <i class="fas fa-check text-success"></i>
                            Sections help organize multiple classes of the same level
                        </li>
                        <li class="mb-2">
                            <i class="fas fa-check text-success"></i>
            ...
            <div class="card grade-card">
                <div class="card-header">
                    <h5 class="card-title"><i class="fas fa-info-circle"></i> Current Class Info</h5>
                </div>
                <div class="card-body">
                    <div class="recent-grades">
                        <div class="recent-grade-item">
                            <div class="grade-badge grade-{{ $grade->id <= 12 ? $grade->id : '1' }}">
                                {{ substr($grade->name, 0, 2) }}
                            </div>
                            <div class="grade-info">
                                <strong>{{ $grade->name }}</strong>
                                <small class="text-muted">
                                    {{ $grade->section ? 'Section ' . $grade->section : 'No section' }} • 
                    ...
                    <div class="mt-3">
                        <small class="text-muted">
                            <strong>Created:</strong> {{ $grade->created_at->format('d M Y') }}<br>
                            <strong>Last Updated:</strong> {{ $grade->updated_at->diffForHumans() }}
                        </small>
                    </div>
                </div>
            </div>
