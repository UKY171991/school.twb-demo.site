...
@section('page-title', 'Modify Institution')
    ...
    <div class="glass-card rounded-3xl overflow-hidden shadow-2xl shadow-indigo-100/50">
        <div class="px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-400 font-medium">Update settings for <strong>{{ $school->name }}</strong></p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.schools.update', $school) }}" class="p-10" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" name="established_date" value="{{ old('established_date', $school->established_date?->format('Y-m-d')) }}"
                                       class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-medium text-slate-700">
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select name="level" required class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-semibold text-slate-700">
                                    ...
                                    <option value="middle" {{ $school->level == 'middle' ? 'selected' : '' }}>Middle School</option>
                                    <option value="high" {{ $school->level == 'high' ? 'selected' : '' }}>High School</option>
                                    <option value="k12" {{ $school->level == 'k12' ? 'selected' : '' }}>K-12</option>
                                </select>
                            </div>
                            <div>
                            ...
                            <div class="flex items-center space-x-4">
                                @if($school->logo)
                                    <div class="w-16 h-16 rounded-2xl border border-slate-200 p-1 bg-white">
                                        <img src="{{ asset('storage/' . $school->logo) }}" class="w-full h-full object-cover rounded-xl shadow-sm">
                                    </div>
                                @endif
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="address" rows="2" 
                                      class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('address', $school->address) }}</textarea>
                        </div>
                            ...
                            <div class="flex items-center space-x-4">
                                @if($school->principal_signature)
                                    <div class="h-12 w-24 rounded-lg border border-slate-200 bg-white p-1">
                                        <img src="{{ asset('storage/' . $school->principal_signature) }}" class="w-full h-full object-contain overflow-hidden">
                                    </div>
                                @endif
                                <div class="flex-1 relative group">
                        ...
                        <div class="flex items-center p-4 bg-indigo-50/30 rounded-2xl border border-indigo-100">
                            <label class="flex items-center cursor-pointer">
                                <div class="relative">
                                    <input type="checkbox" name="is_active" value="1" {{ $school->is_active ? 'checked' : '' }} class="sr-only peer">
            ...
            <div class="mt-10">
                ...
                <textarea name="description" rows="4" 
                          class="w-full px-6 py-5 bg-slate-50 border border-slate-200 rounded-[2rem] focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-medium text-slate-700">{{ old('description', $school->description) }}</textarea>
            </div>
