    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-1">
            <div class="glass-card p-8 rounded-[2rem] shadow-xl sticky top-8 border border-white">
                ...
                <form method="POST" action="{{ route('admin.timetables.exam.store') }}" class="space-y-5">
                    @csrf
                    <div>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/20 transition-colors">
                            <td class="px-8 py-6 text-sm">
                                <div class="font-black text-slate-800">{{ $timetable->exam_date->format('M d, Y') }}</div>
                                <div class="text-[10px] font-bold text-indigo-500 uppercase tracking-wider mt-1">{{ date('H:i', strtotime($timetable->start_time)) }} - {{ date('H:i', strtotime($timetable->end_time)) }}</div>
                            </td>
                            <td class="px-6 py-6 font-bold text-slate-700">{{ $timetable->subject?->name }}</td>
                            <td class="px-6 py-6">
                                <span class="px-3 py-1 bg-slate-100 rounded-lg text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    {{ $timetable->classroom?->name ?? 'TBA' }}
                                </span>
                            </td>
                            <td class="px-6 py-6">
                                <div class="font-black text-slate-800">{{ $timetable->exam_name }}</div>
                                <div class="text-[10px] font-bold text-slate-400 uppercase truncate max-w-[120px]">{{ $timetable->school?->name }}</div>
                            </td>
                        </tr>
                        @empty
                        <tr>
