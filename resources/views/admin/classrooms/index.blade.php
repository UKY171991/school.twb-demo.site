...
@section('page-title', 'Venue Management')
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                        <td class="px-8 py-6">
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="font-black text-slate-700 tracking-tight uppercase text-sm">{{ $classroom->name }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-6 transition-all">
                            <div class="font-bold text-slate-600">{{ $classroom->teacher?->user?->name }}</div>
                            <div class="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{{ $classroom->teacher?->employee_id }}</div>
                        </td>
                        <td class="px-6 py-6">
                            <span class="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase tracking-widest border border-indigo-100">{{ $classroom->subject?->name }}</span>
                        </td>
                        <td class="px-6 py-6 text-center">
                            <div class="inline-flex items-center px-4 py-1.5 bg-slate-50 rounded-2xl border border-slate-100">
                                <span class="text-sm font-black text-slate-800">{{ $classroom->capacity }}</span>
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end space-x-2 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="{{ route('admin.classrooms.edit', $classroom) }}" class="p-3 text-indigo-500 hover:bg-indigo-50 rounded-2xl transition-all" title="Edit Venue">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                </a>
                                <form action="{{ route('admin.classrooms.destroy', $classroom) }}" method="POST" class="confirm-action inline" data-confirm="Decommission venue '{{ $classroom->name }}'?">
                                    @csrf @method('DELETE')
