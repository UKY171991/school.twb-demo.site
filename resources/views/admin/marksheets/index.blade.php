                    ...
                    <tr class="hover:bg-slate-50/20 transition-colors group">
                        <td class="px-8 py-6">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    <div class="font-black text-slate-800 tracking-tight">{{ $student->user->name }}</div>
                                    <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{{ $student->student_id }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-6">
                            @if($student->enrollments->count() > 0)
                                @foreach($student->enrollments as $enrollment)
                                    <span class="inline-block px-3 py-1 bg-slate-100 rounded-full text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{{ $enrollment->classroom?->name }}</span>
                                @endforeach
                            @else
                            ...
                            @if($totalGrades > 0)
                                <div class="flex flex-col">
                                    <div class="flex items-center space-x-2">
                                        <div class="text-sm font-black text-slate-700">{{ number_format($avgGrade, 1) }}%</div>
                                        <div class="flex-1 h-1.5 w-24 bg-slate-100 rounded-full overflow-hidden">
                                            <div class="h-full gradient-primary" style="width: {{ $avgGrade }}%"></div>
                                        </div>
                                    </div>
                                    <div class="text-[9px] font-bold text-slate-400 uppercase tracking-tighter mt-1">{{ $totalGrades }} Assessments recorded</div>
                                </div>
                            @else
                        ...
                        <td class="px-8 py-6 text-right">
                            <a href="{{ route('admin.marksheets.show', $student) }}" class="inline-flex items-center btn-primary py-2 px-4 shadow-md bg-indigo-600 hover:bg-indigo-700 text-xs font-black uppercase tracking-widest rounded-xl transition-all">
