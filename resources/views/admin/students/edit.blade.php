...
@section('page-title', 'Modify Student Profile')
    ...
    <div class="glass-card rounded-3xl overflow-hidden shadow-2xl shadow-blue-100/50">
        <div class="px-10 py-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-slate-400 font-medium">Modifying records for <strong>{{ $student->user->name }}</strong></p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.students.update', $student) }}" class="p-10" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <textarea name="address" rows="3" 
                                      class="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all font-medium text-slate-700">{{ old('address', $student->address) }}</textarea>
                        </div>
                            ...
                            <div class="flex items-center space-x-6 bg-white p-4 rounded-2xl border-2 border-slate-100">
                                @if($student->image)
                                    <div class="relative group">
                                        <img src="{{ asset('storage/' . $student->image) }}" alt="Current image" class="w-16 h-16 rounded-xl object-cover ring-2 ring-blue-100">
