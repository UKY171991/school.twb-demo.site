...
@section('content_header')
    <h1>Edit Exam Timetable Entry</h1>
@stop

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Edit Exam Schedule</h3>
        <div class="card-tools">
            <a href="{{ route('exam-timetables.show', $examTimetable) }}" class="btn btn-info btn-sm">
    ...
    <form action="{{ route('exam-timetables.update', $examTimetable) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="card-body">
            @if($errors->any())
                    ...
                    <div class="form-group">
                        ...
                        <select name="exam_type_id" id="exam_type_id" class="form-control @error('exam_type_id') is-invalid @enderror" required>
                            ...
                            @foreach($examTypes as $examType)
                                <option value="{{ $examType->id }}" {{ $examTimetable->exam_type_id == $examType->id ? 'selected' : '' }}>
                                    {{ $examType->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="subject_id" id="subject_id" class="form-control @error('subject_id') is-invalid @enderror" required>
                            ...
                            @foreach($subjects as $subject)
                                <option value="{{ $subject->id }}" {{ $examTimetable->subject_id == $subject->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <select name="class" id="class" class="form-control @error('class') is-invalid @enderror" required>
                            ...
                            @foreach($grades as $grade)
                                <option value="{{ $grade->name }}" {{ $examTimetable->class == $grade->name ? 'selected' : '' }}>
                                    {{ $grade->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="text" name="section" id="section" class="form-control @error('section') is-invalid @enderror" 
                               value="{{ $examTimetable->section }}" placeholder="e.g., A, B, C (optional)">
                        @error('section')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="academic_year" id="academic_year" class="form-control @error('academic_year') is-invalid @enderror" required>
                            ...
                            @foreach($academicYears as $year)
                                <option value="{{ $year }}" {{ $examTimetable->academic_year == $year ? 'selected' : '' }}>
                                    {{ $year }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input type="date" name="exam_date" id="exam_date" class="form-control @error('exam_date') is-invalid @enderror" 
                               value="{{ $examTimetable->exam_date->format('Y-m-d') }}" required>
                        @error('exam_date')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="time" name="start_time" id="start_time" class="form-control @error('start_time') is-invalid @enderror" 
                               value="{{ $examTimetable->start_time->format('H:i') }}" required>
                        @error('start_time')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="time" name="end_time" id="end_time" class="form-control @error('end_time') is-invalid @enderror" 
                               value="{{ $examTimetable->end_time->format('H:i') }}" required>
                        @error('end_time')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="exam_center" id="exam_center" class="form-control @error('exam_center') is-invalid @enderror" 
                               value="{{ $examTimetable->exam_center }}" placeholder="Will use school name if empty">
                        @error('exam_center')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="is_active" id="is_active" class="form-control @error('is_active') is-invalid @enderror">
                            <option value="1" {{ $examTimetable->is_active ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ !$examTimetable->is_active ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('is_active')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                ...
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea name="instructions" id="instructions" class="form-control @error('instructions') is-invalid @enderror" 
                                  rows="3" placeholder="Any special instructions for this exam">{{ $examTimetable->instructions }}</textarea>
                        @error('instructions')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
        ...
        <div class="card-footer">
            ...
            <a href="{{ route('exam-timetables.show', $examTimetable) }}" class="btn btn-secondary">
